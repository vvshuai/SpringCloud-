 ...
 * @Author: vvshuai
 * @Description:
 * @Date: Created in 22:53 2020/12/5
 * @Modified By:
 */
@Slf4j
@Component
public class LiJianExecutor extends AbstractExecutor implements RuleExecutor {
    ...
    public SettlementInfo computeRule(SettlementInfo settlement) {
        ...
        CouponTemplateSDK templateSDK = settlement.getCouponAndTemplateInfoList().get(0).getTemplateSDK();

        double quota = (double) templateSDK.getRule().getDiscount().getQuota();

        settlement.setCost(
                Math.max(retain2Decimals(goodsSum - quota), doubleMinCost())
        );
